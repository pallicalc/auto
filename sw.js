const CACHE_NAME = 'pallicalc-smart-v19-structure-update'; // Updated to v19

// 1. CRITICAL FILES
// Core App Shell: HTML + Styles + Main JS + Icons
...
const SECONDARY_FILES = [
  ...
  './calculators/calculator.css', // Detected from screenshot
  ...
  // --- EDUCATION FOLDER ASSETS ---
  './education/education.css', // Detected from screenshot
  './education/education.js',  // Detected from screenshot
  ...
  // --- EDUCATION: OTHER TOPICS (HTML ONLY - NO PDFs) ---
  // Bleeding
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    (async () => {
      ...
      // --- 1. EXCLUSION CHECK ---
      // Checks for excluded files and the /Admin/ directory
      ...
      // --- 3. CACHE FIRST (For Assets - including our new Logos) ---
      const cachedResponse = await cache.match(event.request);
      if (cachedResponse) return cachedResponse;
